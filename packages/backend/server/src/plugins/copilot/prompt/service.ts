...
export class PromptService {
  ...
  private describeBuiltInPromptSpec(spec: PromptSpec): ResolvedPrompt {
    ...
    return this.extendChatPromptModels(resolved);
  }

  private extendChatPromptModels(prompt: ResolvedPrompt): ResolvedPrompt {
    if (prompt.name !== 'Chat With AFFiNE AI' || !this.registries) {
      return prompt;
    }
    ...
    const excludedProviderIds = new Set(
      [
        registry.defaults[ModelOutputType.Embedding],
        registry.defaults[ModelOutputType.Rerank],
      ].filter((id): id is string => !!id)
    );
    const configuredModels: string[] = [];

    for (const providerId of registry.order) {
      ...
      if (profile.type === CopilotProviderType.FAL) {
        continue;
      }

      configuredModels.push(
        ...profile.models.map(model => `${providerId}/${model}`)
      );
    }

    const defaultProviderId = registry.defaults[ModelOutputType.Text];
    const defaultProfile = defaultProviderId
      ? registry.profiles.get(defaultProviderId)
      : undefined;
    const defaultModel = defaultProviderId && defaultProfile?.models?.[0]
      ? `${defaultProviderId}/${defaultProfile.models[0]}`
      : prompt.model;
