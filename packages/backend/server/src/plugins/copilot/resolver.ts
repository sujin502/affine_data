...
export class CopilotResolver {
  ...
  private formatModelName(id: string, resolved?: ResolvedCopilotProvider | null) {
    const rawModelId = resolved?.modelId ?? id.split('/').at(-1) ?? id;
    const providerName = resolved?.profile.displayName;
    const modelName = rawModelId
      .split(/[-_.\s/]+/)
      .filter(Boolean)
      .map(part =>
    ...
    const convertModels = async (ids: string[]) => {
      const models = await Promise.all(
        ids.map(async id => {
          ...
          const name =
            resolved.provider.resolveModel(
              resolved.modelId ?? id,
              resolved.execution
            )?.name ?? this.formatModelName(id, resolved);
